...
vec3 UchimuraToneMapping(vec3 x, float exposure) {
  ...
  float cc = 1.33; // black
  float bb = 0.0;  // pedestal
  x *= exposure;
  return uchimura(x, P, a, m, l, cc, bb);
}
...
float Cmax =  50.0; // "Maximum contrast ratio between on-screen luminances" ;
...
float pParam = 200.0;//"Rational mapping curve parameter ;
float Lmax = 500.0;//MaximumLuminance
...
float Lwhite=2.0;  //Smallest luminance that will be mapped to pure white.
...
float  ppParam=1.0;//"Exponent numerator scale factor");
float  qqParam=1.0;//"Exponent denominator scale factor");
...
float pppParam=1.0;//"Exponent numerator scale factor");
float qqqParam=1.0;// "Exponent denominator scale factor");
...
//"Drago Mapping\n\nPropsed in \"Adaptive Logarithmic Mapping For Displaying High Contrast Scenes\" by Drago et al. 2003.";  
 ...
 float Lwmax=2.0;// "MaximumLuminance");
 
vec3 DrogoToneMapping(vec3 color, float exposure ) {
    float LwaP = exposure * Lwa / pow(1.0 + bbbParam - 0.85, 5.0);
    float LwmaxP = exposure * Lwmax / LwaP;
    color *= exposure/ LwaP;
    float L = getLuminance(color);
    float exponent = log(bbbParam) / log(0.5);
    float c1 = (0.01 * Ldmax) / (log(1.0 + LwmaxP)/log(10.0));
    float c2 = log(L + 1.0) / log(2.0 + 8.0 * (pow(L / LwmaxP, exponent)));
    float Ld = c1 * c2;
    color = adjustColor(color, L, Ld);
    return clampedValue(color); 
}
